...
// Replace your existing TwitterFallbackCard function with this enhanced version
function TwitterFallbackCard({ url, tweetId }) {
  const [previewData, setPreviewData] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Try to fetch tweet metadata using a free API or oEmbed
    const fetchTweetPreview = async () => {
      try {
        // Option 1: Try using Twitter's oEmbed API (works for public tweets)
        ...
        if (response.ok) {
          ...
          setPreviewData({
            ...
            description: data.html ? "Click to view tweet" : "Tweet content",
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="microlink-card block w-full max-w-md mb-4 rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:border-blue-400 hover:-translate-y-1 group"
    >
      {/* Twitter/X header */}
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-gray-900 truncate">
              {previewData?.author || "Twitter User"}
            </p>
            <p className="text-xs text-gray-500 truncate">
              @{tweetId ? `status/${tweetId}` : "twitter.com"}
            </p>
          </div>
        </div>
        
        <p className="text-sm text-gray-700 mb-4 line-clamp-3">
          This tweet is no longer available or has been made private.
        </p>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          ...
          <span className="flex items-center gap-1 text-xs font-semibold text-blue-500 group-hover:text-blue-600 transition-colors">
            Open post
            <svg className="w-3 h-3 transform group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
            </svg>
